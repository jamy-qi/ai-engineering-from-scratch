...
- 用几何方式解释点积、投影和 Gram-Schmidt 过程的作用
- 使用行化简确定一组向量的线性无关性、秩和基
- 将线性代数概念与 AI 应用联系起来：嵌入、注意力分数和 LoRA

## 问题

打开任何 ML 论文。在第一页内，你会看到向量、矩阵、点积和变换。没有线性代数直觉，这些只是符号。有了它，你能看到神经网络真正在做什么——在空间中移动点。

你不需要成为数学家。你需要看到这些运算的几何意义，然后自己编写代码。
...
这就是搜索引擎、推荐系统和 RAG 的工作原理——找到点积高的向量。

### 线性无关

如果集合中没有任何向量可以写成其他向量的组合，则这些向量线性无关。如果 v1、v2、v3 无关，它们张成一个 3D 空间。如果其中一个是其他向量的组合，它们只张成一个平面。

为什么对 AI 重要：你的特征矩阵应该有线性无关的列。如果两个特征完全相关（线性相关），模型无法区分它们的影响。这会导致回归中的多重共线性——权重矩阵变得不稳定，小的输入变化会产生剧烈的输出波动。
...
v1 和 v2 无关——两者都不是另一个的标量倍数或组合。但 v3 = 2*v1 + v2，所以 {v1, v2, v3} 是相关集合。这三个向量都在 xy 平面上。无论你怎么组合它们，都无法到达 [0, 0, 1]。你有三个向量但只有两个自由度。
...
3D 空间的标准基是 {[1,0,0], [0,1,0], [0,0,1]}。但 3D 中任意三个无关向量都构成有效基。基的选择就是坐标系的选择。
...
| 秩亏（秩 < min(m, n)） | 低于最大 | 特征冗余。无穷多权重解。需要正则化。 |
...
将任何一组无关向量转换为正交归一基。正交归一意味着每个向量长度为 1，每对向量垂直。
...
这就是 QR 分解的内部工作原理。Q 是正交归一基，R 捕获投影系数。QR 分解用于：
- 求解线性方程组（比高斯消元更稳定）
- 计算特征值（QR 算法）
- 最小二乘回归（标准数值方法）
...
点积对 x 的梯度就是 y。PyTorch 自动计算了这个。神经网络中的每个操作都由这样的操作构建——矩阵乘法、点积、投影——自动微分跟踪所有这些的梯度。

你刚刚从零构建了 NumPy 一行代码做的事情。现在你知道底层发生了什么。

## 输出

本课程产出：
- `outputs/prompt-linear-algebra-tutor.md`——一个让 AI 助手通过几何直觉教授线性代数的提示
...
LoRA 值得特别提及。它通过将权重更新分解为低秩矩阵来微调大语言模型。LoRA 不更新 4096x4096 的权重矩阵（16M 参数），而是更新两个 4096x16 和 16x4096 的矩阵（131K 参数）。秩 16 的约束意味着 LoRA 假设权重更新存在于完整 4096 维空间的 16 维子空间中。这就是线性代数在做真正的工作。

## 练习

1. 实现 `Vector.angle_between(other)`，返回两个向量之间的角度（度）
2. 创建一个 2D 缩放矩阵，将 x 坐标加倍，y 坐标三倍，然后应用到向量 [1, 1]
3. 给定 5 个随机的类词向量（维度 50），用余弦相似度找到最相似的两个
4. 验证 Gram-Schmidt 输出是否真正正交归一：检查每对点积为 0，每个向量大小为 1
5. 创建一个秩为 2 的 3x3 矩阵。用 `rank()` 方法验证。然后解释列张成什么几何对象。
...
| 术语 | 人们怎么说 | 实际意思 |
|------|-----------|---------|
| 向量 | "一个箭头" | 表示 n 维空间中点或方向的数字列表 |
| 矩阵 | "一个数字表" | 将向量从一个空间映射到另一个空间的变换 |
| 点积 | "乘起来再加" | 衡量两个向量对齐程度的度量——相似性搜索的核心 |
| 嵌入 | "一些 AI 魔法" | 表示某物（词、图像、用户）含义的向量 |
| 线性无关 | "它们不重叠" | 集合中没有任何向量可以写成其他向量的组合 |
| 秩 | "多少维度" | 矩阵中线性无关列（或行）的数量 |
| 投影 | "影子" | 一个向量在另一个方向上的分量 |
| 基 | "坐标轴" | 张成空间的最小无关向量集 |
| 正交归一 | "垂直的单位向量" | 互相垂直且每个长度为 1 的向量 |
