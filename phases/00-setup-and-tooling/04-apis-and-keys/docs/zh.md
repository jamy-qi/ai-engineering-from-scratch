...
- 识别和处理常见的 API 错误，包括认证和速率限制

## 问题

从 Phase 11 开始，你将调用 LLM API（Anthropic、OpenAI、Google）。在 Phase 13-16 中，你将构建循环使用这些 API 的智能体。你需要了解 API 密钥如何工作、如何安全存储，以及如何进行第一次 API 调用。
...
response = client.messages.create(
    ...
    max_tokens=256,
    messages=[{"role": "user", "content": "什么是神经网络？用一句话解释。"}]
)
...
| API | 何时需要 | 免费额度 |
|-----|---------|---------|
| Anthropic (Claude) | Phase 11-16（智能体、工具） | 注册送 $5 |
...
| Rate limit | "被限流了" | 每分钟/小时最大请求数，防止滥用并确保公平使用 |
| Token | "一个词"（API 上下文） | 计费单位：输入和输出 token 分别计费 |
